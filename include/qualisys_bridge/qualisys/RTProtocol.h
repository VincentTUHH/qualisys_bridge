...
class DLL_EXPORT CRTProtocol {
 ...
 public:
  ...
  struct SSettingsGeneralExternalTimestamp {
    SSettingsGeneralExternalTimestamp()
        : bEnabled(false), nFrequency(0), nType(Timestamp_SMPTE) {}
    bool bEnabled;
    uint32_t nFrequency;
  ...
  enum EOriginType { GlobalOrigin, RelativeOrigin, FixedOrigin };
  ...
  enum ECalibrationType { regular, fixed, refine };
  ...
  struct SCalibration {
    ...
    double refit_residual =
        std::numeric_limits<double>::quiet_NaN();  // Only for refine
                                                   // calibration.
    double wand_length =
        std::numeric_limits<double>::quiet_NaN();  // Not for fixed calibration.
    uint32_t max_frames = 0;                       // Not for fixed calibration.
    double short_arm_end =
        std::numeric_limits<double>::quiet_NaN();  // Not for fixed calibration.
    double long_arm_end =
        std::numeric_limits<double>::quiet_NaN();  // Not for fixed calibration.
    double long_arm_middle =
        std::numeric_limits<double>::quiet_NaN();  // Not for fixed calibration.
    double result_std_dev =
        std::numeric_limits<double>::quiet_NaN();  // Not for fixed calibration.
    double result_min_max_diff =
        std::numeric_limits<double>::quiet_NaN();  // Not for fixed calibration.
    double result_refit_residual =
        std::numeric_limits<double>::quiet_NaN();  // Only for refine
                                                   // calibration.
 ...
 public:
  ...
  bool Connect(const char* pServerAddr, unsigned short nPort = cDefaultBasePort,
  ...
  bool DiscoverRTServer(
      ...
      unsigned short nDiscoverPort = cDefaultAutoDiscoverPort);
  ...
  [[deprecated("Replaced by ReadGeneralSettings.")]] bool
  ReadCameraSystemSettings();  // Same as ReadGeneralSettings
  ...
  void GetGeneralSettings(unsigned int& nCaptureFrequency, float& fCaptureTime,
  ...
  [[deprecated("Replaced by GetGeneralSettings.")]] void GetSystemSettings(
  ...
  void GetExtTimestampSettings(
      SSettingsGeneralExternalTimestamp& timestamp) const;

  void GetEulerAngles(std::string& first, std::string& second,
  ...
  [[deprecated(
      "EulerNames has been moved to general settings in protocol v1.21. New "
      "accessor is called GetEulerAngles.")]] void
  Get6DOFEulerNames(std::string& first, std::string& second,
                    std::string& third) const;
  ...
  bool SetGeneralSettings(const unsigned int* pnCaptureFrequency,
  ...
  [[deprecated("Replaced by SetGeneralSettings.")]] bool SetSystemSettings(
 ...
 private:
  ...
  void AddXMLElementTransform(CMarkup& xml, const std::string& name,
                              const SPosition& position,
                              const SRotation& rotation);
  void AddXMLElementDOF(CMarkup& xml, const std::string& name,
  ...
  bool ReadXMLDegreesOfFreedom(CMarkup& xml, const std::string& element,
                               std::vector<SDegreeOfFreedom>& degreesOfFreedom);
