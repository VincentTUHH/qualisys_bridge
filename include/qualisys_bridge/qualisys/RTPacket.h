...
class DLL_EXPORT CRTPacket {
 ...
 public:
  ...
  unsigned int GetEyeTrackerSampleNumber(
      unsigned int eyeTrackerIndex);  // Returns 0 if no sample was found.
  bool GetEyeTrackerData(unsigned int eyeTrackerIndex,
                         unsigned int nSampleIndex, SEyeTracker &nEyeTracker);
  bool GetEyeTrackerData(unsigned int eyeTrackerIndex,
                         SEyeTracker *pEyeTrackerBuf, unsigned int nBufSize);
  ...
  bool GetTimecodeIRIG(int &years, int &days, int &hours, int &minutes,
