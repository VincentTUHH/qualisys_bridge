...
class CNetwork {
 public:
  enum class ResponseType { success, timeout, disconnect, error };

  struct Response {
    int received;
    ResponseType type;

    Response(ResponseType type_, int received_)
        : received(received_), type(type_) {}
  ...
  Response Receive(char* rtDataBuff, int nDataBufSize, bool bHeader,
                   int timeoutMicroseconds, unsigned int* ipAddr = nullptr);
  Response ReceiveUdpBroadcast(char* rtDataBuff, int nDataBufSize,
 ...
 private:
  Response Receive(SOCKET socket, SOCKET udpSocket, char* rtDataBuff,
                   int nDataBufSize, bool bHeader, int timeoutMicroseconds,
                   unsigned int* ipAddr = nullptr);
  bool InitWinsock();
  void SetErrorString();
