...
CNetwork::Response CNetwork::Receive(SOCKET socket, SOCKET udpSocket,
                                     char* rtDataBuff, int dataBufSize,
                                     bool header, int timeoutMicroseconds,
                                     unsigned int* ipAddr) {
  int received = 0;
  ...
  if (FD_ISSET(socket, &exceptFDs)) {
    ...
    received = recvfrom(udpSocket, rtDataBuff, dataBufSize, 0,
...
CNetwork::Response CNetwork::Receive(char* rtDataBuff, int dataBufSize,
                                     bool header, int timeoutMicroseconds,
                                     unsigned int* ipAddr) {
  return Receive(mSocket, mUDPSocket, rtDataBuff, dataBufSize, header,
                 timeoutMicroseconds, ipAddr);
}

CNetwork::Response CNetwork::ReceiveUdpBroadcast(char* rtDataBuff,
                                                 int dataBufSize,
                                                 int timeoutMicroseconds,
                                                 unsigned int* ipAddr) {
  return Receive(static_cast<SOCKET>(SOCKET_ERROR), mUDPBroadcastSocket,
                 rtDataBuff, dataBufSize, false, timeoutMicroseconds, ipAddr);
}
