        ...
        if (!mp2DData.empty()) {
          ...
          for (nCamera = 1; nCamera < mn2DCameraCount; nCamera++) {
            if (mnMajorVersion > 1 || mnMinorVersion > 7) {
              mp2DData[nCamera] = mp2DData[nCamera - 1] + 5 +
                                  Get2DMarkerCount(nCamera - 1) * 12;
            } else {
              mp2DData[nCamera] = mp2DData[nCamera - 1] + 4 +
                                  Get2DMarkerCount(nCamera - 1) * 12;
            }
          }
        }
      ...
      if (nComponentType == ComponentForceSingle) {
        ...
        if (!mpForceSingleData.empty()) {
          ...
          for (nDevice = 1; nDevice < mnForceSinglePlateCount; nDevice++) {
            mpForceSingleData[nDevice] =
                mpForceSingleData[nDevice - 1] + 4 + 36;
          }
        }
      }
      ...
      if (nComponentType == ComponentEyeTracker) {
        ...
        if (!mpEyeTrackerData.empty()) {
          ...
          for (nDevice = 1; nDevice < mnEyeTrackerCount; nDevice++) {
            unsigned int nPrevSampleCount =
                SetByteOrder((unsigned int*)(mpEyeTrackerData[nDevice - 1]));
            mpEyeTrackerData[nDevice] = mpEyeTrackerData[nDevice - 1] + 4 +
                                        ((nPrevSampleCount == 0) ? 0 : 4) +
                                        nPrevSampleCount * 28;
          }
        }
  ...
  if (mnMajorVersion > 1 || mnMinorVersion > 7) {
    ...
    for (int i = 0; i < 9; i++) {
      afRotMatrix[i] = (float)SetByteOrder(
          (double*)(pData + 40 + (i * 8) + nBodyIndex * 96));
    }
  }
  return true;
}
...
//                       Eye Tracker
//-----------------------------------------------------------
unsigned int CRTPacket::GetEyeTrackerCount() { return mnEyeTrackerCount; }
...
bool CRTPacket::GetEyeTrackerData(unsigned int eyeTrackerIndex,
                                  unsigned int nSampleIndex,
                                  SEyeTracker& sEyeTracker) {
  ...
  for (unsigned int k = 0; k < (sizeof(SEyeTracker) / sizeof(float)); k++) {
    *(((float*)&sEyeTracker) + k) = (float)SetByteOrder(
        (float*)(mpEyeTrackerData[eyeTrackerIndex] + 8 + k * sizeof(float) +
                 nSampleIndex * sizeof(SEyeTracker)));
  }
  ...
  for (unsigned int nSample = 0; nSample < nSampleCount; nSample++) {
    for (unsigned int k = 0; k < (sizeof(SEyeTracker) / sizeof(float)); k++) {
      *(((float*)pEyeTrackerBuf) + k + (nSample * sizeof(SEyeTracker))) =
          (float)SetByteOrder((float*)(mpEyeTrackerData[eyeTrackerIndex] + 8 +
                                       k * sizeof(float) +
                                       nSample * sizeof(SEyeTracker)));
    }
  }
...
//                       Timecode
//-----------------------------------------------------------
bool CRTPacket::IsTimeCodeAvailable() const { return mnTimecodeCount > 0; }
...
bool CRTPacket::GetTimecodeType(CRTPacket::ETimecodeType& timecodeType) {
  ...
  timecodeType = (CRTPacket::ETimecodeType)SetByteOrder(
      (unsigned int*)(mpTimecodeData[0]));
  return true;
}

bool CRTPacket::GetTimecodeSMPTE(int& hours, int& minutes, int& seconds,
                                 int& frames) {
  if (mnTimecodeCount <= 0) {
    return false;
  }
  ...
  if (GetTimecodeType(timecodeType)) {
    if (timecodeType == TimecodeSMPTE) {
      ...
      minutes =
          0x3f & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8)) >> 5);
      seconds =
          0x3f & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8)) >> 11);
      frames =
          0x1f & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8)) >> 17);
      return true;
    }
  }
  return false;
...
bool CRTPacket::GetTimecodeIRIG(int& years, int& days, int& hours, int& minutes,
  ...
  if (GetTimecodeType(timecodeType)) {
    if (timecodeType == TimecodeIRIG) {
      ...
      days =
          0x01ff & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 4)) >> 7);
      hours = 0x001f & SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8));
      minutes =
          0x003f & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8)) >> 5);
      seconds =
          0x003f & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8)) >> 11);
      tenths =
          0x000f & (SetByteOrder((unsigned int*)(mpTimecodeData[0] + 8)) >> 17);
      return true;
    }
  }
  return false;
...
bool CRTPacket::GetTimecodeCameraTime(unsigned long long& cameraTime) {
  ...
  if (GetTimecodeType(timecodeType)) {
    if (timecodeType == TimecodeCamerTime) {
      cameraTime =
          ((long long)SetByteOrder((unsigned int*)(mpTimecodeData[0] + 4)))
              << 32 |
