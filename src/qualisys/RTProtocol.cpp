...
static const auto DEGREES_OF_FREEDOM = {
    ...
    std::make_pair(CRTProtocol::EDegreeOfFreedom::TranslationZ,
                   "TranslationZ")};
}  // namespace
  ...
  *((unsigned short*)(pData + 8)) = htons(nServerPort);
  ...
  if (mpoNetwork->SendUDPBroadcast(pData, 10, nDiscoverPort)) {
    ...
    do {
      ...
      response = mpoNetwork->ReceiveUdpBroadcast(
          mDataBuff.data(), (int)mDataBuff.size(), 100000, &nAddr);

      if (response && response.received > qtmPacketHeaderSize) {
        if (CRTPacket::GetType(mDataBuff.data()) == CRTPacket::PacketCommand) {
          char* discoverResponse =
              CRTPacket::GetCommandString(mDataBuff.data());
          sResponse.nAddr = nAddr;
          sResponse.nBasePort =
              CRTPacket::GetDiscoverResponseBasePort(mDataBuff.data());
          ...
          if (discoverResponse &&
            ...
            mvsDiscoverResponseList.push_back(sResponse);
             ...
             response.received >
                 qtmPacketHeaderSize);  // Keep reading until no more responses.
    ...
    if (bC3D) {
      ...
      if (SendCommand((mnMajorVersion > 1 || mnMinorVersion > 7)
        ...
        if (strcmp(pResponseStr, "Sending capture") == 0) {
          if (Receive(eType, true, 5000000) ==
              CNetwork::ResponseType::success)  // Wait for C3D file in 5
                                                // seconds.
          {
      ...
      if (SendCommand("GetCaptureQTM", pResponseStr)) {
        if (strcmp(pResponseStr, "Sending capture") == 0) {
          if (Receive(eType, true, 5000000) ==
              CNetwork::ResponseType::success)  // Wait for QTM file in 5
                                                // seconds.
          {
...
bool CRTProtocol::LoadCapture(const char* pFileName) {
  ...
  if (strlen(pFileName) <= 250) {
    ...
    if (SendCommand(sendString.c_str(), pResponseStr,
...
bool CRTProtocol::GetEventString(CRTPacket::EEvent eEvent, char* pStr) {
  switch (eEvent) {
    ...
    case CRTPacket::EventCalibrationStopped:
      strcpy(pStr, "Calibration Stopped");
      break;
...
std::vector<std::pair<unsigned int, std::string>> CRTProtocol::GetComponents(
    const std::string componentsString) {
...
CNetwork::ResponseType CRTProtocol::Receive(CRTPacket::EPacketType& eType,
                                            bool bSkipEvents, int nTimeout) {
  CNetwork::Response response(CNetwork::ResponseType::error, 0);
  ...
  do {
    ...
    response = mpoNetwork->Receive(mDataBuff.data(), (int)mDataBuff.size(),
                                   true, nTimeout);
    ...
    if (eType == CRTPacket::PacketC3DFile ||
      ...
      if (mpFileBuffer != nullptr) {
        ...
        if (fwrite(mDataBuff.data() + sizeof(int) * 2, 1,
        ...
        while (nRecvedTotal < nFrameSize) {
          ...
          response = mpoNetwork->Receive(&(mDataBuff.data()[sizeof(int) * 2]),
                                         nReadSize, false, cWaitForDataTimeout);
          if (response.type == CNetwork::ResponseType::timeout) {
          ...
          if (fwrite(mDataBuff.data() + sizeof(int) * 2, 1, response.received,
                     mpFileBuffer) != (size_t)(response.received)) {
            strcpy(maErrorStr, "Failed to write file to disk.");
            fclose(mpFileBuffer);
            mpFileBuffer = nullptr;
            return CNetwork::ResponseType::error;
      ...
      while (nRecvedTotal < nFrameSize) {
        // As long as we haven't received enough data, wait for more
        response = mpoNetwork->Receive(&(mDataBuff.data()[nRecvedTotal]),
                                       nFrameSize - nRecvedTotal, false, -1);
        if (response.type == CNetwork::ResponseType::timeout) {
          strcpy(maErrorStr, "Packet truncated.");
          return CNetwork::ResponseType::error;
        }
...
retry:
  ...
  if (eType != CRTPacket::PacketXML) {
    if (eType == CRTPacket::PacketError) {
      ...
      goto retry;
      // sprintf(maErrorStr, "GetParameters %s returned wrong packet type. Got
      // type %d expected type 2.", settingsType.c_str(), eType);
    }
  }
...
bool CRTProtocol::ReadCameraSystemSettings() { return ReadGeneralSettings(); }
...
bool CRTProtocol::ReceiveCalibrationSettings(int timeout) {
  ...
  } while (event != CRTPacket::EventNone &&
           event != CRTPacket::EventConnectionClosed);
  ...
  try {
    ...
    if (settings.type != ECalibrationType::fixed) {
      ...
      if (settings.type == ECalibrationType::refine) {
        settings.result_refit_residual =
            std::stod(oXML.GetChildAttrib("refit-residual"));
        settings.result_consecutive =
            std::stoul(oXML.GetChildAttrib("consecutive"));
      }
    }
    ...
    if (mnMajorVersion > 1 || mnMinorVersion > 20) {
      while (oXML.FindChildElem("Body")) {
        ...
        if (mnMajorVersion > 1 || mnMinorVersion > 23) {
          ...
          s6DOFBodySettings.enabled =
              oXML.GetChildData() == "true" ? true : false;
        }
        ...
        s6DOFBodySettings.color = (colorR & 0xff) | ((colorG << 8) & 0xff00) |
                                  ((colorB << 16) & 0xff0000);

        if (!oXML.FindChildElem("MaximumResidual")) {
          return false;
        }
        s6DOFBodySettings.maxResidual =
            (float)atof(oXML.GetChildData().c_str());
        ...
        s6DOFBodySettings.boneLengthTolerance =
            (float)atof(oXML.GetChildData().c_str());
        ...
        if (oXML.FindChildElem("Mesh")) {
          ...
          s6DOFBodySettings.mesh.position.fX =
              (float)atof(oXML.GetChildAttrib("X").c_str());
          s6DOFBodySettings.mesh.position.fY =
              (float)atof(oXML.GetChildAttrib("Y").c_str());
          s6DOFBodySettings.mesh.position.fZ =
              (float)atof(oXML.GetChildAttrib("Z").c_str());

          if (!oXML.FindChildElem("Rotation")) {
            return false;
          }
          s6DOFBodySettings.mesh.rotation.fX =
              (float)atof(oXML.GetChildAttrib("X").c_str());
          s6DOFBodySettings.mesh.rotation.fY =
              (float)atof(oXML.GetChildAttrib("Y").c_str());
          s6DOFBodySettings.mesh.rotation.fZ =
              (float)atof(oXML.GetChildAttrib("Z").c_str());

          if (!oXML.FindChildElem("Scale")) {
            return false;
          }
          s6DOFBodySettings.mesh.scale =
              (float)atof(oXML.GetChildData().c_str());

          if (!oXML.FindChildElem("Opacity")) {
            return false;
          }
          s6DOFBodySettings.mesh.opacity =
              (float)atof(oXML.GetChildData().c_str());
        ...
        if (oXML.FindChildElem("Points")) {
          ...
          while (oXML.FindChildElem("Point")) {
            ...
            sBodyPoint.virtual_ =
                (0 != atoi(oXML.GetChildAttrib("Virtual").c_str()));
            sBodyPoint.physicalId =
                atoi(oXML.GetChildAttrib("PhysicalId").c_str());
        ...
        s6DOFBodySettings.origin.type =
            (EOriginType)atoi(oXML.GetChildData().c_str());
        s6DOFBodySettings.origin.position.fX =
            (float)atof(oXML.GetChildAttrib("X").c_str());
        s6DOFBodySettings.origin.position.fY =
            (float)atof(oXML.GetChildAttrib("Y").c_str());
        s6DOFBodySettings.origin.position.fZ =
            (float)atof(oXML.GetChildAttrib("Z").c_str());
        s6DOFBodySettings.origin.relativeBody =
            atoi(oXML.GetChildAttrib("Relative_body").c_str());

        if (!oXML.FindChildElem("Data_orientation")) {
          return false;
        }
        if (s6DOFBodySettings.origin.type !=
                atoi(oXML.GetChildData().c_str()) ||
            s6DOFBodySettings.origin.relativeBody !=
        ...
        for (uint32_t i = 0; i < 9; i++) {
          ...
          s6DOFBodySettings.origin.rotation[i] =
              (float)atof(oXML.GetChildAttrib(tmpStr).c_str());
        }
...
bool CRTProtocol::ReadGazeVectorSettings(bool& bDataAvailable) {
  ...
  while (oXML.FindChildElem("Vector")) {
    ...
    mvsGazeVectorSettings.push_back(
        {tGazeVectorName, frequency, hwSync, filter});
    nGazeVectorCount++;
...
bool CRTProtocol::ReadSkeletonSettings(bool& dataAvailable,
    ...
    if (mnMajorVersion > 1 || mnMinorVersion > 20) {
      while (xml.FindElem("Skeleton")) {
        ...
        if (xml.FindElem("Segments")) {
          ...
          std::function<void(SSettingsSkeletonSegmentHierarchical&,
                             std::vector<SSettingsSkeletonSegment>&, const int)>
              recurseSegments =
                  [&](SSettingsSkeletonSegmentHierarchical& segmentHierarchical,
                    ...
                    if (xml.FindElem("Transform")) {
                      xml.IntoElem();
                      segmentHierarchical.position =
                          ReadXMLPosition(xml, "Position");
                      segmentHierarchical.rotation =
                          ReadXMLRotation(xml, "Rotation");
                      xml.OutOfElem();  // Transform
                    }

                    if (xml.FindElem("DefaultTransform")) {
                      xml.IntoElem();
                      segmentHierarchical.defaultPosition =
                          ReadXMLPosition(xml, "Position");
                      segmentHierarchical.defaultRotation =
                          ReadXMLRotation(xml, "Rotation");
                    ...
                    segmentHierarchical.endpoint =
                        ReadXMLPosition(xml, "Endpoint");
                    ...
                    if (xml.FindElem("Markers")) {
                      ...
                      while (xml.FindElem("Marker")) {
                        ...
                        xml.IntoElem();
                        marker.position = segmentHierarchical.endpoint =
                            ReadXMLPosition(xml, "Position");
                    ...
                    segment.parentIndex =
                        (parentId != -1) ? segmentIdIndexMap[parentId] : -1;
                    segment.positionX =
                        (float)segmentHierarchical.defaultPosition.x;
                    segment.positionY =
                        (float)segmentHierarchical.defaultPosition.y;
                    segment.positionZ =
                        (float)segmentHierarchical.defaultPosition.z;
                    segment.rotationX =
                        (float)segmentHierarchical.defaultRotation.x;
                    segment.rotationY =
                        (float)segmentHierarchical.defaultRotation.y;
                    segment.rotationZ =
                        (float)segmentHierarchical.defaultRotation.z;
                    segment.rotationW =
                        (float)segmentHierarchical.defaultRotation.w;

                    segments.push_back(segment);

                    while (xml.FindElem("Segment")) {
                      SSettingsSkeletonSegmentHierarchical childSegment;
                      recurseSegments(childSegment, skeleton.segments,
                                      segmentHierarchical.id);
          ...
          if (xml.FindElem("Segment")) {
            recurseSegments(skeletonHierarchical.rootSegment, skeleton.segments,
                            -1);
          }
      ...
      while (xml.FindElem("Skeleton")) {
        ...
        while (xml.FindElem("Segment")) {
          ...
          if (segment.name.size() == 0 ||
              sscanf(xml.GetAttrib("ID").c_str(), "%u", &segment.id) != 1) {
            return false;
          }
          ...
          int parentId;
          if (sscanf(xml.GetAttrib("Parent_ID").c_str(), "%d", &parentId) !=
              1) {
            segment.parentId = -1;
...
CRTProtocol::SPosition CRTProtocol::ReadXMLPosition(
    CMarkup& xml, const std::string& element) {
  SPosition position;
...
CRTProtocol::SRotation CRTProtocol::ReadXMLRotation(
    CMarkup& xml, const std::string& element) {
  SRotation rotation;
...
bool CRTProtocol::ReadXMLDegreesOfFreedom(
    CMarkup& xml, const std::string& element,
...
void CRTProtocol::GetSystemSettings(
    ...
    EProcessingActions& eRtProcessingActions,
    EProcessingActions& eReprocessingActions) const {
  GetGeneralSettings(nCaptureFrequency, fCaptureTime, bStartOnExtTrig,
                     startOnTrigNO, startOnTrigNC, startOnTrigSoftware,
                     eProcessingActions, eRtProcessingActions,
...
void CRTProtocol::GetExtTimestampSettings(
    SSettingsGeneralExternalTimestamp& timestamp) const {
  timestamp = msGeneralSettings.sTimestamp;
}

void CRTProtocol::GetEulerAngles(std::string& first, std::string& second,
...
std::vector<CRTProtocol::SSettingsGeneralCamera> CRTProtocol::GetDevices()
    const {
...
bool CRTProtocol::Get6DOFBodySettings(
    std::vector<SSettings6DOFBody>& settings) {
  if (mnMajorVersion == 1 && mnMinorVersion < 21) {
    strcpy(maErrorStr,
           "Get6DOFBodySettings not available before protocol version 1.21");
    return false;
  }
...
bool CRTProtocol::GetGazeVectorHardwareSyncUsed(
    unsigned int nGazeVectorIndex) const {
  if (nGazeVectorIndex < mvsGazeVectorSettings.size()) {
...
const char* CRTProtocol::GetEyeTrackerName(
    unsigned int nEyeTrackerIndex) const {
...
bool CRTProtocol::GetEyeTrackerHardwareSyncUsed(
    unsigned int nEyeTrackerIndex) const {
  if (nEyeTrackerIndex < mvsEyeTrackerSettings.size()) {
...
bool CRTProtocol::GetSkeleton(unsigned int skeletonIndex,
                              SSettingsSkeletonHierarchical& skeleton) {
...
void CRTProtocol::GetSkeletons(
    std::vector<SSettingsSkeletonHierarchical>& skeletons) {
...
bool CRTProtocol::SetSystemSettings(
  ...
  return SetGeneralSettings(pnCaptureFrequency, pfCaptureTime, pbStartOnExtTrig,
                            startOnTrigNO, startOnTrigNC, startOnTrigSoftware,
                            peProcessingActions, peRtProcessingActions,
                            peReprocessingActions);
}
...
bool CRTProtocol::SetExtTimestampSettings(
    const CRTProtocol::SSettingsGeneralExternalTimestamp& timestampSettings) {
  CMarkup oXML;
...
bool CRTProtocol::Set6DOFBodySettings(std::vector<SSettings6DOFBody> settings) {
  if (mnMajorVersion == 1 && mnMinorVersion < 21) {
    strcpy(maErrorStr,
           "Set6DOFBodySettings only available for protocol version 1.21 and "
           "later.");
  ...
  for (auto& body : settings) {
    ...
    oXML.AddElem("MinimumMarkersInBody",
                 std::to_string(body.minMarkersInBody).c_str());
    oXML.AddElem("BoneLengthTolerance",
                 std::to_string(body.boneLengthTolerance).c_str());
    oXML.AddElem("Filter");
    ...
    oXML.AddAttrib("Relative_body",
                   std::to_string(body.origin.relativeBody).c_str());
...
bool CRTProtocol::SetSkeletonSettings(
    const std::vector<SSettingsSkeletonHierarchical>& skeletons) {
  CMarkup xml;
  ...
  for (auto& skeleton : skeletons) {
    ...
    std::function<void(const SSettingsSkeletonSegmentHierarchical)>
        recurseSegments = [&](const SSettingsSkeletonSegmentHierarchical&
                                  segment) {
          xml.AddElem("Segment");
          ...
          xml.IntoElem();
          {
            ...
            if (!std::isnan(segment.position.x)) {
              AddXMLElementTransform(xml, "Transform", segment.position,
                                     segment.rotation);
            }
            if (!std::isnan(segment.defaultPosition.x)) {
              AddXMLElementTransform(xml, "DefaultTransform",
                                     segment.defaultPosition,
                                     segment.defaultRotation);
            }
            ...
            {
              if (!std::isnan(segment.endpoint.x) &&
                  !std::isnan(segment.endpoint.y) &&
                  !std::isnan(segment.endpoint.z)) {
            ...
            xml.IntoElem();
            {
              for (const auto& rigidBody : segment.bodies) {
                ...
                xml.AddElem("Position");
                xml.AddAttrib("X",
                              std::to_string(rigidBody.position.x).c_str());
                xml.AddAttrib("Y",
                              std::to_string(rigidBody.position.y).c_str());
                xml.AddAttrib("Z",
                              std::to_string(rigidBody.position.z).c_str());
                xml.AddElem("Rotation");
                xml.AddAttrib("X",
                              std::to_string(rigidBody.rotation.x).c_str());
                xml.AddAttrib("Y",
                              std::to_string(rigidBody.rotation.y).c_str());
                xml.AddAttrib("Z",
                              std::to_string(rigidBody.rotation.z).c_str());
                xml.AddAttrib("W",
                              std::to_string(rigidBody.rotation.w).c_str());
                xml.OutOfElem();  // Transform
...
const char* CRTProtocol::SkeletonDofToString(
    CRTProtocol::EDegreeOfFreedom dof) {
  auto it = std::find_if(DEGREES_OF_FREEDOM.begin(), DEGREES_OF_FREEDOM.end(),
                         [&](const auto& DEGREE_OF_FREEDOM) {
                           return (DEGREE_OF_FREEDOM.first == dof);
                         });
...
CRTProtocol::EDegreeOfFreedom CRTProtocol::SkeletonStringToDof(
    const std::string& str) {
  auto it = std::find_if(DEGREES_OF_FREEDOM.begin(), DEGREES_OF_FREEDOM.end(),
                         [&](const auto& DEGREE_OF_FREEDOM) {
                           return (DEGREE_OF_FREEDOM.second == str);
                         });
...
bool CRTProtocol::SendString(const char* pCmdStr, int nType) {
  ...
  uint32_t nSize =
      8 + nCmdStrLen +
      1;  // Header size + length of the string + terminating null char
...
void CRTProtocol::AddXMLElementTransform(CMarkup& xml, const std::string& name,
...
void CRTProtocol::AddXMLElementDOF(CMarkup& xml, const std::string& name,
                                   SDegreeOfFreedom degreeOfFreedoms) {
  xml.AddElem(name.c_str());
  if (!std::isnan(degreeOfFreedoms.lowerBound) &&
      !std::isnan(degreeOfFreedoms.upperBound)) {
    if (mnMajorVersion > 1 || mnMinorVersion > 21) {
      xml.IntoElem();
      xml.AddElem("Constraint");
    }
    xml.AddAttrib("LowerBound",
                  std::to_string(degreeOfFreedoms.lowerBound).c_str());
    xml.AddAttrib("UpperBound",
                  std::to_string(degreeOfFreedoms.upperBound).c_str());
  }

  if (std::isnan(degreeOfFreedoms.lowerBound) ||
      std::isnan(degreeOfFreedoms.upperBound) ||
      (mnMajorVersion == 1 && mnMinorVersion < 22)) {
    xml.IntoElem();
  }
  ...
  if (!degreeOfFreedoms.couplings.empty()) {
    ...
    xml.IntoElem();
    {
      for (const auto& coupling : degreeOfFreedoms.couplings) {
        ...
        xml.AddAttrib("DegreeOfFreedom",
                      SkeletonDofToString(coupling.degreeOfFreedom));
        xml.AddAttrib("Coefficient",
                      std::to_string(coupling.coefficient).c_str());
      }
    }
    xml.OutOfElem();  // Couplings
  }

  if (!std::isnan(degreeOfFreedoms.goalValue) &&
      !std::isnan(degreeOfFreedoms.goalWeight)) {
    xml.AddElem("Goal");
    xml.AddAttrib("Value", std::to_string(degreeOfFreedoms.goalValue).c_str());
    xml.AddAttrib("Weight",
                  std::to_string(degreeOfFreedoms.goalWeight).c_str());
  }
  xml.OutOfElem();
}
